...
import { getInfosByFileIds } from '/@/api/ad/advertise';
import { usePaging } from './usePaging';
import { ElMessage, ElMessageBox, ElTree, type CheckboxValueType } from 'element-plus';
...
// 处理文件的钩子函数
export function useFile(cateId: Ref<string | number>, type: Ref<number>, current: Ref<number>, size: number) {
	...
	const batchFileDelete = async (id?: number[]) => {
		...
		try {
			const res = await getInfosByFileIds({ fileIds: ids });
			if(res?.ok && res?.data.length > 0) {
				ElMessageBox.alert(
					`<div style='color: red;'>${res.data.map((item: any) => item).join(',')}</div>`,
					'以下素材已被投放不可删除！',
					{
						type: 'warning',
				...
				useMessageBox().confirm('确认删除后本地将同步删除，如文件已被使用，请谨慎操作！').then(async () => {
	...
	const selectFile = (item: any) => {
		...
		if (select.value.length == current.value) {
			if (current.value == 1) {
				select.value = [];
