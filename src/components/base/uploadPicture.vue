<template>
  <div style="min-width: 850px">
    ...
    <el-row :gutter="20" v-loading="loadingPic">
      <el-col v-bind="grid" :style="{ width: !pictureType ? '23%' : '17%' }">
        <div class="Nav">
                    ...
                    <span class="el-ic">
                      <el-dropdown>
                        ...
                        <el-dropdown-menu slot="dropdown">
                          <el-dropdown-item @click.native="onAdd(data.id)" v-if="node.label !== '全部图片'"
                            >添加分类</el-dropdown-item
                          >
                          <el-dropdown-item v-if="node.label !== '全部图片'" @click.native="onEdit(data.id)"
                            >编辑分类</el-dropdown-item
                          >
                          <el-dropdown-item v-if="node.label !== '全部图片'" @click.native="handleOpenDelete(data.id)"
      ...
      <el-col v-bind="grid2" :style="{ width: !pictureType ? '77%' : '83%' }">
            ...
            >
              ...
              >
                ...
                <div class="operate" v-if="pictureType && item.isCk">
                  <div class="btn btn-primary" @click="onPreview(item)">预览</div>
                  <div class="btn btn-danger ml-10" @click="deleteOperate(item)">删除</div>
    ...
    <PreviewPictureDialog ref="previewRef" />
  </div>
</template>
...
import PreviewPictureDialog from '../common/previewPictureDialog.vue';
...
export default {
  ...
  created() {
    ...
    this.pictureType ? (this.tableData.size = 18) : (this.tableData.size = 18);
  ...
  mounted() {
    if (this.pictureType) {
      ...
      this.resizeObserve.observe(this.$refs.myDiv);
  ...
  destroyed() {
    ...
    if(this.resizeObserve) {
      this.resizeObserve.disconnect(this.$refs.myDiv);
    }
  ...
  methods: {
    //视口大小变化更改盒子宽度
    // getWindowInfo() {
    //   console.log('RESIZE改变图片库页面宽度', this.divWidth);
    //   this.divWidth = this.$refs.myDiv.offsetWidth;
    // },
    allAdd(label) {
      if (label == '全部图片') {
    ...
    // 图片预览
    onPreview(data) {
      // console.log("图片或视频预览", data);
      // 本地测试 - 可播放的视频文件
      // if(data.attId == 2727) {
      //   data.sattDir = "http://vjs.zencdn.net/v/oceans.mp4"
      // } else if(data.attId == 2717) {
      //   data.sattDir = "http://www.w3school.com.cn/example/html5/mov_bbb.mp4"
      // }

      this.$refs.previewRef.openDialog({ previewUrl: data.sattDir, type: data.attType });
    },
    //移入移出显示操作
    ...
    getFileListAsync() {
      return new Promise((resolve, reject) => {
        ...
        fileListApi(this.tableData)
          .then(async (res) => {
            this.pictrueList.list = res.records;
            if (this.tableData.current === 1 && this.pictrueList.list.length > 0)
