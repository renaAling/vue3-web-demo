<template>
	<el-form @submit.prevent :model="formModel" v-bind="_options" ref="formRef">
		<template v-for="(item, index) in fieldList" :key="index">
			...
			<!-- 日期时间组件 -->
			<el-form-item :label="item.label" :rules="item.rules" :prop="[item.field]" v-else-if="item.type === 'date' || item.type === 'datetime'">
				<DatePicker v-model="item.type" />
			</el-form-item>
			<!-- 默认输入框 -->
...
// 引入组件
const DatePicker = defineAsyncComponent(() => import('../DatePicker/index.vue'));
