		...
		<template v-for="(c, index) in columns" :key="index">
			<el-form-item :label="c.label" v-if="c.search" :prop="(c.prop as string)">
				...
				<el-date-picker
					...
					style="width: 200px"
				/>
				<el-input v-else v-model="model[c.prop as string]" :placeholder="'请输入' + c.label" clearable style="width: 200px" />
			</el-form-item>
		</template>
		<el-form-item>
			<el-button :loading="loading" type="primary" @click="emit('search', model)" :icon="Search">搜索</el-button>
			<el-button :loading="loading" :icon="Refresh" @click="searchForm?.resetFields()">重置</el-button>
		</el-form-item>
	</el-form>
</template>
