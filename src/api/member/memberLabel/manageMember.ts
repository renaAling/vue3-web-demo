...
export const saveOrUpdateUserInfos = (params?: Object) => {
	return request({
		url: '/uac/memberSignInfo/saveOrUpdateUserInfos',
		method: 'post',
		data: params,
	});
};
...
export const getUserInfos = (uid?: string) => {
	return request({
		url: `/uac/memberSignInfo/getUserInfos?uid=${uid}`,
		method: 'get',
	});
};

// 支持模糊查询获取所有会员标签(绑定会员下拉使用)
export const getMemberSignList = (name?: string) => {
	return request({
		url: `/uac/memberSign/getList?name=${name}`,
		method: 'get',
	});
};
