...
service.interceptors.request.use(
	(config: InternalAxiosRequestConfig) => {
		...
		config.headers['rls-version'] = `${year}${month}${day}`;
		// 统一增加TENANT-ID请求头
		const tenantId = config.headers['TENANT-ID'] || Session.getTenant();
		...
		if (merchantId) {
			config.headers![CommonHeaderEnum.MERCHANT_ID] = merchantId;
		}
