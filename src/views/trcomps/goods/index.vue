		...
		<el-card shadow="never" class="mb8" :body-style="{ paddingBottom: 0 }">
			<el-form :inline="true" :model="state.queryForm" @keyup.enter="getDataList" ref="queryRef">
				<el-form-item label="商品名称" prop="keywords">
					<el-input placeholder="请输入商品名称" v-model="state.queryForm.keywords" clearable />
				</el-form-item>
				<el-form-item label="状态" prop="isShow">
					<el-select v-model="state.queryForm.isShow" clearable>
		...
		<div class="layout-padding-auto layout-padding-view">
			<el-row>
				<div class="mb8" style="width: 100%">
					<el-button type="info" plain @click="batchDelisting('up')" :disabled="selectObjs.length == 0">上架</el-button>
					<el-button type="info" plain @click="batchDelisting('down')" :disabled="selectObjs.length == 0">下架</el-button>
					<el-button type="info" plain @click="batchDelisting('delete')" :disabled="selectObjs.length == 0">删除</el-button>
				</div>
			</el-row>
			<el-table
				:data="state.dataList"
			...
			>
				...
				<el-table-column label="状态" prop="isShow">
					<template #default="{ row }">
						<div v-if="row.isShow == 1">上架中</div>
						<div v-if="row.isShow == 0">已下架</div>
					</template>
				</el-table-column>
...
import { pageList, batchRecycle, batchDown, batchUp, updateSort, getRefreshTime, refreshList } from '/@/api/goodStore/goods';
