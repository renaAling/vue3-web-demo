			...
			>
				...
				<el-table-column label="状态" prop="status" show-overflow-tooltip>
					<template #default="scope">
						<div :class="scope.row.state ? 'text-[#67c23a]' : 'text-[#f56c6c]'">{{ scope.row.state ? '开启' : '关闭' }}</div>
					</template>
				</el-table-column>
...
import { pageList, updateStatus } from '/@/api/pay/channel';
import { useMessage, useMessageBox } from '/@/hooks/message';
import { useI18n } from 'vue-i18n';
import { useDict } from '/@/hooks/dict';

// 引入组件
const FormDialog = defineAsyncComponent(() => import('./channelForm.vue'));
const { t } = useI18n();
// 定义变量内容
...
const beforeChange = async (row: any) => {
	const msg = row.state ? '确定要关闭该账户吗?' : '确定要开启该账户吗?';
	try {
