<template>
	<el-dialog v-model="state.upload.open" :title="prop.title" draggable :close-on-click-modal="false">
		<el-upload
			ref="uploadRef"
			:limit="1"
			accept=".xlsx, .xls"
	...
	<el-dialog title="批量导入结果" v-model="state.resultVisible" width="40%">
		<div class="content">
			<div>导入完成</div>
			<div>导入条数{{ state.totalNum }}条</div>
			<div>成功{{ state.successNum }}条</div>
			<div>失败{{ state.failNum }}条</div>
		</div>
		<el-table :data="state.failItems" border>
			<el-table-column prop="serialNumber" label="序号" />
			<el-table-column prop="words" label="敏感词" />
...
const handleFileSuccess = (response: any) => {
	...
	if (response.code === 1) {
		...
		// 无论接口返回成功或者失败都需要弹出
		const { totalNum, successNum, failNum, failExcels, isFail, failItems } = response.data;
