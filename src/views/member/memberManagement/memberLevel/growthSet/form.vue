<template>
	<el-dialog :close-on-click-modal="false" :title="!form.id ? '新增' : '修改'" width="800" draggable v-model="visible" :before-close="resetForm">
		<el-form :model="form" label-width="110px" ref="dataFormRef" :rules="dataRules" v-loading="loading">
			...
			<el-form-item label="发放成长值" prop="growthValue">
...
import { postMemberGrowthManageUpdate, postMemberGrowthManageAdd } from '/@/api/member/memberLevel/growthSet';
...
const onSubmit = async () => {
	...
	try {
		loading.value = true;
		const { code, msg } = form.id ? await postMemberGrowthManageUpdate(form) : await postMemberGrowthManageAdd(form);
		if (code === 0) {
