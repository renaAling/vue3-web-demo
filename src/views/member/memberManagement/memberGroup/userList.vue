...
import {
	pageMemberGroupInfoDel,
	pageMemberGroupInfoPage,
	getUserInfoByUnionIdForMember,
	pageMemberGroupInfoUpDate,
...
const queryRef = ref();
const importUrl = ref(`/uac/memberGroupInfo/import?groupCode=${route.query.code}`);
...
const handleUpDate = async (row: any, userInfoData: any) => {
	try {
		const { code, data } = await pageMemberGroupInfoUpDate({ id: row.id, mobile: userInfoData.mobile });
		if (code === 0) {
			console.log('data', data);
...
const handelDel = (id: string) => {
	useMessageBox()
		...
		.then(async () => {
			try {
				const { code } = await pageMemberGroupInfoDel(Number(id));
				if (code === 0) {
...
const excelUpload = () => {
	if (Number(route.query.status) === 2) {
		useMessage().error('该分组已禁用，不能进行批量导入');
...
const state: BasicTableProps = reactive<BasicTableProps>({
	...
	pageList: pageMemberGroupInfoPage,
	descs: ['create_time'],
});
