						...
						<div class="flex-1 px-2">
							<el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="86" @keydown.enter.prevent>
								...
								<el-row :gutter="20">
									<el-col :span="8" class="mb20">
										<div class="flex justify-start items-center text-slate-500">
											<div class="text-right" style="width: 72px">授信余额:</div>
					...
					<div class="mx-2">
						<el-form :inline="true" :model="queryForm" @keyup.enter="getDetailsChanges" ref="queryRef">
							...
							<el-form-item label="流水号" prop="serialNumber">
								<el-input v-model="queryForm.serialNumber" placeholder="请输入流水号" />
							</el-form-item>
							<el-form-item label="类型" prop="eventType">
								<el-select v-model="queryForm.eventType" clearable style="width: 180px">
						...
						>
							...
							<el-table-column label="类型" prop="transactionType">
								<template #default="{ row }">
									<span v-if="row.transactionType == 1">支出</span>
									<sapn v-if="row.transactionType == 2">退款</sapn>
									<span v-if="row.transactionType == 3">充值</span>
								</template>
							</el-table-column>
...
import { getMemberUserCreditLogGet, getMemberUserCreditLogPage } from '/@/api/member/priceAccount/creditAccount';
...
const getUserInfo = async (id: string) => {
	try {
		const { code, data, msg } = await getMemberUserCreditLogGet(id);
		if (code === 0) {
...
const getDetailsChanges = async () => {
	...
	try {
		const { code, data, msg } = await getMemberUserCreditLogPage(params);
		if (code === 0) {
