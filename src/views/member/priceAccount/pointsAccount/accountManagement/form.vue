<template>
	<el-dialog :close-on-click-modal="false" title="调整积分" width="600" draggable v-model="visible">
		<el-form :model="form" label-width="78px" ref="dataFormRef" :rules="dataRules" v-loading="loading">
			...
			<el-form-item label="积分" prop="changeScore">
				<el-input-number v-model="form.changeScore" :precision="2" :step="0.1" step-strictly :min="0" />
			</el-form-item>
...
const dataRules = ref({
	...
	manualType: [{ required: true, message: '类型不能为空', trigger: 'blur' }],
	changeScore: [{ required: true, message: '金额不能为空', trigger: 'blur' }],
