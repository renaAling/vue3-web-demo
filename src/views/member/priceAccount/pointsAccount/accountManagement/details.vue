						...
						<div class="flex-1 px-2">
							<el-form ref="dataFormRef" :model="dataForm" :rules="dataRules" label-width="86" @keydown.enter.prevent>
								...
								<el-row :gutter="20">
									<el-col :span="8" class="mb20">
										<div class="flex justify-start items-center text-slate-500">
											<div class="text-right" style="width: 72px">积分余额:</div>
											<span class="ml-2 text-slate-700">{{ dataForm.price ? dataForm.price : '-' }}</span>
										</div>
									</el-col>
									<el-col :span="8" class="mb20">
										<div class="flex justify-start items-center text-slate-500">
				...
				<div class="mr-5 py-2 bg-white rounded border-inherit border border-solid boxSizing">
					...
					<div class="mx-2">
						<el-form :inline="true" :model="queryForm" @keyup.enter="getDetailsChanges" ref="queryRef">
							...
							<el-form-item label="流水号" prop="scoreSerialNumber">
								<el-input v-model="queryForm.scoreSerialNumber" placeholder="请输入流水号" />
							</el-form-item>
							<el-form-item label="类型" prop="type">
								<el-select v-model="queryForm.type" clearable style="width: 180px">
							...
							<el-form-item label="渠道" prop="channelCode">
								<el-select v-model="queryForm.channelCode" clearable style="width: 180px">
									<el-option v-for="(item, key) in channelListData" :key="key" :label="item.name" :value="item.channelCode"></el-option>
								</el-select>
							</el-form-item>
							<el-form-item>
					...
					<div class="mx-2">
						...
						>
							...
							<el-table-column label="使用渠道" prop="channelName"></el-table-column>
...
import { getDetailForMemberApi, getFlowPage } from '/@/api/member/priceAccount/pointsAccount';
...
watch(
	...
	(val) => {
		if (val?.length > 0) {
			queryForm.value.handleStartTime = val[0];
			queryForm.value.handleEndTime = val[1];
		} else {
			queryForm.value.handleStartTime = '';
			queryForm.value.handleEndTime = '';
		}
	}
);
const show = (row: any) => {
	...
	unionId.value = '';
	if (row) {
		unionId.value = row.unionId;
		getDetailsChanges(); //变动明细
		// Object.assign(dataForm, row);
		getDetailForMember(); //获取详情
	}
};

const getDetailForMember = async () => {
	try {
		const { code, data, msg } = await getDetailForMemberApi(unionId.value);
		if (code === 0) {
...
const getDetailsChanges = async () => {
	...
	try {
		const { code, data, msg } = await getFlowPage(params);
		if (code === 0) {
