			...
			<el-row class="ml10">
				<el-form :inline="true" :model="state.queryForm" @keyup.enter="getDataList" ref="queryRef">
					...
					<el-form-item label="商家类型" prop="merchantType">
						<el-select v-model="state.queryForm.merchantType" clearable>
...
const handleCommand = async (command: Table.Command, row: any = {}) => {
	switch (command) {
		...
		case 'delete':
			...
			break;
		case 'add':
			router.push('/goodStore/merchantManage/QPMerchants/config');
			break;
		case 'edit':
			router.push({ path: '/goodStore/merchantManage/QPMerchants/config', query: { id: row.id } });
			break;
		case 'justment':
