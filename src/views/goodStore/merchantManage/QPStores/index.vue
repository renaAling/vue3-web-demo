	...
	<div class="layout-padding">
		<div class="layout-padding-auto layout-padding-view">
			...
			>
				...
				<el-table-column label="推荐状态" prop="isRecommend">
					<template #default="{ row }" >
...
import { storePageApi, storeUnbindApi, storeUpdateStatusApi, storeDelApi, storeUpdateSortApi, updateRecommendStatusApi } from '/@/api/trcomps/store';
...
const beforeChangeRecommend = async (row: any) => {
	const { code, msg } = await updateRecommendStatusApi({
		id: row.id,
...
const handleCommand = async (command: Table.Command, row: any = {}) => {
	switch (command) {
		...
		case 'downCode':
			...
			break;
		case 'add':
			router.push('/goodStore/merchantManage/QPStores/config');
			break;
		case 'edit':
			router.push({ path: '/goodStore/merchantManage/QPStores/config', query: { id: row.id, storeCode: row.storeCode } });
			break;
	}
};
</script>
