<template>
	<div>
		<el-dialog :title="dialogTitle" ref="myDialog" v-model="visible" draggable width="80%" :close-on-click-modal="false" :show-close="false">
			<el-form @submit.prevent :model="model" ref="formRef" label-width="180px" :rules="formRules">
				<el-tabs v-model="activeName" class="demo-tabs">
					<el-tab-pane label="基本信息" name="Base">
						...
						<el-form-item label="跳转链接" prop="jumpUrl" v-if="!isMerchant && merchantType == 3">
							<el-input
...
import { getCategoryList, getSubwayList, merchantList, storeConfig, storeDetail, subwayList, addAndEdit, getByCode } from '/@/api/goodStore/merchant';
...
const openDialog = async (id: string | null) => {
	...
	if (id) {
		...
		model.value.mapLongitude = Number(originLocationInfo.lng);
		model.value.mapLatitude = Number(originLocationInfo.lat);
...
const handleMerchantChange = (val: string) => {
	merchantType.value = merchantsList.value.find((item) => item?.value === val)?.type;
};
