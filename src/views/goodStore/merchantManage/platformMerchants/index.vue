<template>
	<div class="layout-padding">
		<div class="layout-padding-auto layout-padding-view">
			...
			<el-row>
				<div class="mb8" style="width: 100%">
					<el-button @click="handleCommand('add')" icon="folder-add" type="primary">新建</el-button>
				</div>
			</el-row>
			<el-table
				:data="state.dataList"
			...
			>
				...
				<!-- <el-table-column label="商家渠道" prop="storeNum" /> -->
				<el-table-column label="商家ID" prop="merchantCode" />
				<el-table-column label="商家名称" prop="merchantName" />
				<el-table-column label="商家类型" prop="type">
					<template #default="{ row }">
						<el-tag>{{ businessTypelist[row.type] }}</el-tag>
					</template>
				</el-table-column>
				<el-table-column label="押金（元）" prop="depositMoneyLabel" />
		...
		<businessDig ref="businessRef" @refresh="getDataList()"></businessDig>
	</div>
</template>
...
const businessDig = defineAsyncComponent(() => import('./businessConfig.vue'));
// 字典
...
const handleCommand = async (command: Table.Command, row: any = {}) => {
	switch (command) {
		...
		case 'delete':
			...
			break;
		case 'add':
			router.push('/goodStore/merchantManage/platformMerchants/config');
			break;
		case 'edit':
			router.push({ path: '/goodStore/merchantManage/platformMerchants/config', query: { id: row.id } });
			break;
		case 'config':
			businessRef.value.openDialog(row);
			break;
	}
};
</script>
