			...
			>
				...
				<el-table-column label="操作" width="240">
					<template #default="{ row }">
						...
						<el-button type="primary" link icon="lock" @click="bindDialogRef.openDialog(row.id)" v-if="!row.merchantName && !isMerchant"
							>绑定商家</el-button
						>
						<el-button type="primary" link icon="unlock" @click="handleCommand('unbind', row)" v-if="row.merchantName && !isMerchant"
							>解绑商家</el-button
						>
						<el-button type="primary" link icon="download" @click="handleCommand('downCode', row)">下载收款码</el-button>
						<el-button type="primary" link icon="delete" @click="handleCommand('delete', row)" v-if="!isMerchant">删除</el-button>
					</template>
				</el-table-column>
			</el-table>
...
import { storeDel, storePage, storeUnbind, storeUpdateDisplayStatus, storeUpdateSortApi } from '/@/api/goodStore/merchant';
