	...
	<div class="layout-padding">
		<div class="layout-padding-auto layout-padding-view">
			<el-row shadow="hover" v-show="showSearch" class="ml10">
				<el-form :model="state.queryForm" ref="queryRef" :inline="true" @keyup.enter="getDataList">
					...
					<el-form-item label="发放时间" prop="distributeTime">
						<el-date-picker v-model="state.queryForm.distributeTime" type="date" value-format="YYYY-MM-DD HH:mm:ss" />
					</el-form-item>
					<el-form-item label="使用时间" prop="useTime">
						<el-date-picker v-model="state.queryForm.useTime" type="date" value-format="YYYY-MM-DD HH:mm:ss" />
					</el-form-item>
					<el-form-item>
			...
			>
				...
				<el-table-column prop="validTime" label="发放时间" show-overflow-tooltip></el-table-column>
				<el-table-column prop="useTime" label="使用时间" show-overflow-tooltip></el-table-column>
				<el-table-column prop="status" label="券码状态" show-overflow-tooltip>
					<template #default="{ row }">
						<span>{{ typeArr.find((item: any) => item.value == row.status)?.label }}</span>
					</template>
				</el-table-column>
...
import { pageCouponUserInfoPage, downloadRecord } from '/@/api/goodStore/coupon';
const queryRef = ref();
const showSearch = ref(true);
const typeArr = ref([]) as any;
const state: BasicTableProps = reactive<BasicTableProps>({
	queryForm: {},
	pageList: pageCouponUserInfoPage, // H
	descs: [],
});
onMounted(() => {
	geTypeArr();
...
const handleExport = async () => {
	...
	await downloadRecord(params, `使用记录.xlsx`);
};
