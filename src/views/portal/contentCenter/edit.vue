...
import { addContent, contentDetail, allChannel, updateContent, getUpdateArticleChannelList } from '/@/api/portal';
...
onMounted(async () => {
	...
	if (id) {
		...
		if (code === 0) {
			dataForm.value = data;
			// dataForm.value.columnId = [data.channelId, data.columnId];
		}
	}
...
const onSubmit = async () => {
	...
	try {
		...
		if (code === 0) {
			...
			// router.go(-1);
			router.replace({
				path: '/portal/contentCenter/index',
				query: { active: '2' },
			});
		}
	} catch (error: any) {
