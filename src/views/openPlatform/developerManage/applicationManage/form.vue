	...
	<div class="layout-padding">
		...
		<section>
			<el-card>
				...
				<el-button @click="handleGoToInterfaceAuth" type="primary" style="margin-bottom: 20px">接口授权</el-button>
				<el-table border :data="model.authorizes" :cell-style="tableStyle?.cellStyle" :header-cell-style="tableStyle?.headerCellStyle" >
					...
					<el-table-column label="操作">
						<template #default="{ row }">
							<el-button type="text" @click="handleRemoveToBlackList(row)">移入黑名单</el-button>
						</template>
					</el-table-column>
				</el-table>
			</el-card>
		...
		<!-- 移入黑名单弹窗 -->
		<el-dialog v-model="blackFormdialogVisible" title="移入黑名单" width="410" align-center>
			<el-form ref="formRef" :model="blackForm" label-width="auto">
				<el-form-item label="接口名称">
					<el-input v-model="blackForm.interfaceName" disabled></el-input>
				</el-form-item>
				<el-form-item label="应用名称">
					<el-input v-model="blackForm.appName" disabled></el-input>
				</el-form-item>
				<el-form-item label="操作说明">
					<el-input v-model="blackForm.remark" type="textarea"></el-input>
				</el-form-item>
			</el-form>
			<template #footer>
				<div class="dialog-footer">
...
const handleRemoveToBlackList = (row: any) => {
	blackFormdialogVisible.value = true;
	blackForm.value.interfaceName = row.interfaceName;
	blackForm.value.appName = model.value.appName;
	blackForm.value.id = row.id;
};
...
const handleGoToInterfaceAuth = () => {
	router.push({
		path: '/openPlatform/developerManage/applicationManage/interfaceAuth',
		query: {
