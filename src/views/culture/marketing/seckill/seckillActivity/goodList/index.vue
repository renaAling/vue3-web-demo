<template>
  <div>
    ...
    <el-pagination :page-sizes="[10, 20, 30, 40]" :page-size="state.tableFrom.size"
      :current-page="state.tableFrom.current" layout="total, sizes, prev, pager, next, jumper"
...
const changeType = (v) => {
  state.isChecked = v;
  const index = state.checkedPage.indexOf(state.tableFrom.current);
  state.isIndex = index;
  state.checkedPage.push(state.tableFrom.current);
  if (index > -1) {
...
const getList = (num) => {
  ...
  state.tableFrom.current = num ? num : state.tableFrom.current;
...
const pageChange = (current) => {
  state.tableFrom.current = current;
  getList();
};
const handleSizeChange = (val) => {
  state.tableFrom.size = val;
  getList();
};
</script>
