		...
		<div class="layout-padding-auto layout-padding-view">
			<el-row class="ml10">
				<el-form :inline="true" :model="state.queryForm" @keyup.enter="getDataList" ref="queryRef">
					<el-form-item label="姓名" prop="name">
						<el-input placeholder="请输入姓名查询" v-model="state.queryForm.name" clearable />
					</el-form-item>
			...
			<el-row>
				<div class="mb8" style="width: 100%">
					<el-button @click="formDialogRef.openDialog()" class="ml10" icon="folder-add" type="primary" v-auth="'sys_post_add'">新增</el-button>
				</div>
			</el-row>
			<el-table
				:data="state.dataList"
			...
			>
				...
				<el-table-column label="手机号" prop="sjh" show-overflow-tooltip />
...
<script lang="ts" name="supplierManageUser" setup>
...
const handleCommand = async (command: Table.Command, row: any) => {
	switch (command) {
		case 'unlock': // 启用
			try {
				await useMessageBox().confirm('请确认是否启用员工', '启用');
			} catch {
				return;
			}
		...
		case 'lock': // 停用
			try {
				...
				if (upSpuNum > 0 || afterSaleNum > 0) {
					await useMessageBox().confirm(`是否停用员工`, '停用');
				}
			} catch {
				return;
			}
		...
		case 'delete':
			try {
				await useMessageBox().confirm('请确认是否将 员工 删除，删除动作无法恢复，请谨慎操作。', '删除');
			} catch {
				return;
			}
