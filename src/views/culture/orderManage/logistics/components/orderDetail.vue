					...
					<ul class="list">
						<li class="item">
							...
							<div class="color-warning" v-if="orderDatalist.shippingType === 2">
								<!-- 订单自提核销  -->
								<span v-if="orderDatalist.status === 0">待支付</span>
								<span v-if="orderDatalist.status === 1">待核销</span>
								<span v-if="orderDatalist.status === 2">待核销</span>
							...
							<div class="color-warning" v-if="orderDatalist.shippingType == 5">
								...
								<span v-if="orderDatalist.status === 2">部分发货</span>
								<span v-if="orderDatalist.status === 4">配送中</span>
								<span v-if="orderDatalist.status === 5">已收货</span>
								<span v-if="orderDatalist.status === 6">已完成</span>
								<span v-if="orderDatalist.status === 9">已取消</span>
								<span v-if="orderDatalist.status === 10">已退款</span>
				...
				<el-tabs type="border-card" v-model="activeName">
                  ...
                  <div class="value">{{ orderDatalist.merCouponPrice || '0.0' }}</div>
						...
						<div class="detailSection">
							...
							<div v-if="Number(orderDatalist.status) > 1">
								<ul class="list" v-for="(item, index) of orderDatalist.invoiceResponses || []" :key="index">
									<li class="item">
										<div class="tips" v-if="index === 0">快递公司：</div>
									...
									<li class="item">
										<div class="tips" v-if="index === 0">物流单号：</div>
										<div class="tips" v-else></div>
										<div class="value">
											{{ item.trackingNumber || '-' }}
										</div>
							...
							<div v-else>-</div>
							<!-- <div style="line-height: 26px" v-if="Number(orderDatalist.status) > 2">
								<div class="flex" style="align-items: center"><span class="mr-20">快递公司:</span><span>物流单号：</span></div>
								<div>配送状态：</div>
							</div>
							<div v-else>-</div> -->
							<!-- <ul class="list">
                  ...
                  <div class="acea-row row-middle">
					...
					<el-tab-pane
						v-if="
							...
							orderDatalist?.type < 5 &&
							orderDatalist.shippingType != 5 &&
					...
					<el-tab-pane label="售后记录" name="operationLog">
						<el-table
							...
							:data="orderDatalist?.refundDetailList || []"
							border
							:cell-style="{ textAlign: 'left' }"
							:header-cell-style="{ textAlign: 'left', background: '#e7eefd', color: '#515a6e' }"
						>
					...
					<el-tab-pane label="操作记录" name="refundDetail">
						<el-table
							class="mt20 orderDetailList"
					...
					<el-tab-pane label="核销记录" name="verification">
						<el-table
							class="mt20"
							:data="orderDatalist?.orderVerifyList || []"
							border
							:cell-style="{ textAlign: 'left' }"
							:header-cell-style="{ textAlign: 'left', background: '#e7eefd', color: '#515a6e' }"
						>
							<el-table-column label="核销人员" prop="verifyName" />
							<el-table-column label="核销人员手机号" prop="verifyPhone" />
							<el-table-column label="核销人员时间" prop="verifyTime" />
							<el-table-column label="核销结果" prop="scorePrice">
								<template #default="{ row }">
									<div>已核销</div>
								</template>
							</el-table-column>
