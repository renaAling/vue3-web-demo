				...
				<div class="detailHead">
					<div class="acea-row row-between headerBox">
						...
						<div class="acea-row row-center-wrapper">
							<!-- 审核 -->
							<div v-if="refundInfo.refundStatus === 0 || refundInfo.refundStatus == 7" class="acea-row row-center-wrappe">
								<el-button
									type="primary"
									@click="
										() => {
					...
					<ul class="list">
						...
						<li class="item" v-if="tenantId == '1922177185280557058'">
							<div class="title">退款积分</div>
							<div v-if="refundInfo.refundStatus == 0"><!--待审核状态不展示退款金额--></div>
							<div v-else>{{ refundInfo.refundScorePrice || '0.0' }}</div>
						</li>
						<li class="item" v-if="tenantId == '1922177185280557058'">
							<div class="title">实付积分</div>
							<div>{{ refundInfo.scorePrice }}</div>
						</li>
						<li class="item">
				...
				<el-tabs type="border-card" v-model="activeName">
						...
						<div class="detailSection">
							...
							<ul class="list">
								<li class="item">
									...
									<div class="value" v-if="tenantId != '1922177185280557058'">￥{{ refundInfo.orderInfoVo.proTotalPrice }}</div>
									<div class="value" v-if="tenantId == '1922177185280557058'">{{ refundInfo.orderInfoVo.scorePrice }}</div>
								</li>
								<li class="item" v-if="tenantId != '1922177185280557058'">
                  ...
                  <div class="value">￥{{ refundInfo.orderInfoVo.platCouponPrice }}</div>
								...
								<li class="item">
									...
									<div class="value" v-if="tenantId != '1922177185280557058'">￥{{ refundInfo.orderInfoVo.payPrice }}</div>
									<div class="value" v-if="tenantId == '1922177185280557058'">{{ refundInfo.orderInfoVo.scorePrice }}</div>
								</li>
								<li class="item" v-if="tenantId != '1922177185280557058'">
								...
								<li class="item">
									...
									<div class="value" v-if="tenantId != '1922177185280557058'">￥{{ refundInfo.orderInfoVo.svipDiscountPrice || 0 }}</div>
									<div class="value" v-if="tenantId == '1922177185280557058'">{{ refundInfo.orderInfoVo.svipDiscountScorePrice || 0 }}</div>
								</li>
								<li class="item" v-if="tenantId != '1922177185280557058'">
					...
					<el-tab-pane label="商品信息" name="goods">
						<el-table
							border
							class="mt20"
							:data="refundInfo.orderDetailList"
								...
								<template #default="{ row }">
									<div class="acea-row row-middle">
										<div class="demo-image__preview mr15 line-heightOne">
											<el-image class="w-[100px] h-[100px]" :src="row.image" :preview-src-list="[row.image]" />
										</div>
...
import { expressInfoApi, refundMarkApi, refundOrderDetailApi, auditAfterSaleApi, operatorAuditApi } from '/@/api/culture/order';
...
// 验证规则
const afterRules = {
	reason: [{ required: true, message: '请输入审核说明', trigger: ['blur', 'change'] }],
...
// 计算总价 = 价格 + 积分换算
const calculatePrice = (price: number, score: number) => {
	const p = price || 0;
...
// 获取退款金额
const getMoney = () => {
	if (refundPriceInfo.value.isAll) {
		return refundPriceInfo.value.totalPrice ?? 0;
	} else {
		return refundInfo.value.refundPrice;
...
const getRefundScore = () => {
	if (refundPriceInfo.value.isAll) {
		...
		return refundInfo.value.refundScorePrice;
...
const submitForm = async (formEl: FormInstance | undefined) => {
	...
	await formEl.validate(async (valid, fields) => {
		if (valid) {
			console.log('submit!');
			try {
				const { code, msg } = await refundMarkApi(form.value);
				if (code === 0) {
	...
	await formEl.validate(async (valid, fields) => {
		if (valid) {
			...
			try {
				...
				if (code === 0) {
					useMessage().success('操作成功');
					resetExpressageForm(ruleFormRef.value);
				} else {
					useMessage().error(msg);
				}
			} catch (err: any) {
