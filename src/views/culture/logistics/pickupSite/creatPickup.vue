			...
			<el-form ref="ruleFormRef" :model="ruleForm" label-width="100px" size="small" :rules="rules">
				...
				<el-card>
					...
					<el-form-item label="所属供应商：" prop="merId">
						<merchantName class="withs" ref="merchantNameRef" @getMerId="getMerId" :merIdChecked="ruleForm.merId" :platId="ruleForm.goodsType" />
					</el-form-item>
...
const merchantName = defineAsyncComponent(() => import('../../commodity/commodityList/components/merchantName/index.vue'));
...
const rules = {
	...
	merId: [{ required: true, message: '请选择所属供应商', trigger: 'change' }],
	...
	jsonp(`https://apis.map.qq.com/ws/geocoder/v1`, {
		...
		.then((res: any) => {
			...
			if (status === 0) {
				...
				ruleForm.adcode = result.ad_info.adcode;
...
const onsubmit = (formEl: any) => {
	...
	if (ruleForm.adcode) {
		if (ruleForm.adcode != ruleForm.districtCode) {
			useMessage().error('请选择和详细地址匹配的省市区');
			return;
		}
	}
