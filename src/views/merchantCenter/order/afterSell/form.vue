		...
		<el-dialog title="售后详情" ref="myDialog" v-model="visible" width="40%" :close-on-click-modal="false">
			<el-form :model="data">
				<el-card class="mb-5">
					...
					<el-form-item label="退回金额">{{ data.refundTotalAmount }}元</el-form-item>
...
const data = ref({
	...
	discountAmount: null,
	wechatReturnedAmount: null,
	cardReturnedAmount: null,
	refundCode: '',
	orderCode: '',
	refundStatus: null,
	userId: '',
	...
	if (row.id) {
		...
		try {
			...
			if (res.code === 0) {
				...
				const operator = `${res.data.operator} (${res.data.mobile})`;
