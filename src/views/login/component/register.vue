	...
	<el-form size="large" class="login-content-form" :rules="dataRules" ref="dataFormRef" :model="state.ruleForm">
		...
		<el-form-item>
			<el-checkbox v-model="state.ruleForm.checked">
				<span class="text-gray-400">{{ $t('password.readAccept') }}</span>
			</el-checkbox>
			<span class="text-primary hover:text-blue-600">
		...
		<el-form-item class="login-animation4">
			<el-button type="primary" class="login-content-submit rounded-lg" v-waves @click="handleRegister" :loading="loading">
				<span class="tracking-wide font-semibold">{{ $t('password.registerBtnText') }}</span>
			</el-button>
		</el-form-item>

		<div class="relative flex items-center justify-between">
			<div class="text-sm ml-auto">
				<a href="#" class="text-primary hover:text-blue-600" @click="emit('change', LoginTypeEnum.PASSWORD)"> 返回登录 </a>
			</div>
		</div>
	</el-form>
</template>
