...
import { pageList, useMenuApi } from '/@/api/admin/menu';
...
export async function initBackEndControlRoutes() {
	...
	console.log('userInfos', userInfos.value.authBtnList);
	// 获取有没有管理模式权限
	const isAdminModel = userInfos.value.authBtnList.includes('management_mode');
	Session.set('isAdminModel',isAdminModel)
	// 获取路由菜单数据
	const res = await getBackEndControlRoutes(isAdminModel);
	...
	// if (!isAdminModel && Session.get('adminModel')) {
	// 	console.log('重定向', import.meta.env.VITE_REDIRECT_URL);
...
console.log('modeView.value', modeView.value);

export function getBackEndControlRoutes(isAdminModel?: boolean) {
	const adminModel = Session.get('adminModel');
	// return pageList();
	const roleIds = Session.get('roleIds');
	const params = {
		roleIds,
		// menuType: modeView.value === 'admin' ? 3 : undefined, // 管理模式时，menuType 为 3
		menuType: adminModel === 'admin' && isAdminModel ? 3 : undefined, // 管理模式时，menuType 为 3
	};
